   ...
   **/
   function showTabsContent($options=array()) {
      ...
      if (count($onglets)) {
         ...
         //#HOLDAT Gambs com relacao a mostrar as abas de 'reserva' 'admin'

         $tabs    = array();

         $switch_reservation = false; // #GAMBIARRA

         if ($onglets['ReservationItem$1'] == 'Reserva') {  // Checa se a pagina se trata da pagina de reserva, entao ativa um booleano
            $switch_reservation = true;
         }

         if ($switch_reservation) {                   // Com o booleano ativado, passa fixo apenas uma 'tab'
            $tabs['ReservationItem$1'] = array('title' => 'Reserva',
                'url' => $tabpage,
                'params' => "_target=$target&amp;_itemtype=" . $this->getType() .
                    "&amp;_glpi_tab=ReservationItem$1&amp;id=$ID$extraparamhtml");
         } else {
         ...
         Ajax::createTabs('tabspanel', 'tabcontent', $tabs, $this->getType(), $ID,
                          $this->taborientation, $switch_reservation);
      }
      echo "</div>";
   }
      ...
      if ($this->get_item_to_display_tab) {
         if (isset($_GET["id"]) && $_GET["id"] && !$this->can($_GET["id"], READ)) {
            ...
            if (isset($_SESSION['_redirected_from_profile_selector'])
               ...
               Html::redirect($CFG_GLPI['root_doc']."/front/reservation.php");
            }
